...
function drawWire(input,marker)
  ...
  for section = input;
    ...
    section_direction = section{1}(1);
    switch section_direction
      case 'L'
        dx = -1;
        dy = 0;
      case 'R'
   ...
   section_length = str2num(substr(section{1},2));
...
function [distance_closest, distances] = findClosestIntersection()
  
  global grid;
  global hub_x;
  global hub_y;
  
  [intersections_x, intersections_y] = find(grid == 99);
  
  distances = abs(intersections_x - hub_x) + abs(intersections_y - hub_y);
  distance_closest = min(distances);
    
endfunction
 
function [least_steps, intersection_steps] = findLeastSteps()
  ...
  for i = 1:length(intersections_x)
    intersection_steps(i) = steps_1(intersections_x(i),intersections_y(i)) + steps_2(intersections_x(i),intersections_y(i));
  endfor
  
  least_steps = min(intersection_steps);
...
function testClosest()
  ...
  % test 1
  grid = zeros(width,height,"uint8");
  steps_1 = zeros(width,height,"uint16");
  steps_2 = zeros(width,height,"uint16");
  grid(hub_x, hub_y) = 1;
  ...
  if findClosestIntersection() == 159
    passed += 1;
  endif
  ...
  if findClosestIntersection() == 135
    passed += 1;
  endif
  
  if passed == 3
    printf("distance test: ok\r\n");
  else
...
function testSteps()
  ...
  if findLeastSteps() == 410
    passed += 1;
  endif
  
  if passed == 3
    printf("steps test: ok\r\n");
  else
    printf("steps test: failed\r\n");
  endif
...
printf("Advent of Code - 2019 - day 03\r\n");
printf("part 1:\r\n");
...
grid = zeros(width,height,"uint8");
steps_1 = zeros(width,height,"uint32");
steps_2 = zeros(width,height,"uint32");
grid(hub_x, hub_y) = 1;
...
[least_steps, intersection_steps] = findLeastSteps();
